    ...
    <main className="min-h-screen bg-[#050505] px-4 py-8 text-white sm:px-6 sm:py-12">
      <div className="mx-auto w-full max-w-2xl">
        <header className="mb-8 text-center">
          <div className="mx-auto mb-5 flex h-16 w-16 items-center justify-center rounded-2xl border border-[#d4af37]/35 bg-[#d4af37]/10 shadow-[0_0_50px_rgba(212,175,55,0.12)]">
        ...
        <section className="rounded-3xl border border-white/10 bg-white/[0.035] p-5 shadow-2xl shadow-black/30 backdrop-blur sm:p-8">
          {checking ? (
            <div className="flex min-h-72 flex-col items-center justify-center gap-3 text-white/70">
              <Loader2 className="h-8 w-8 animate-spin text-[#d4af37]" />
              <p>Validando tu invitación…</p>
            </div>
          ) : !validLink ? (
            <div className="flex min-h-72 flex-col items-center justify-center text-center">
              <ShieldCheck className="mb-4 h-10 w-10 text-white/35" />
              <h2 className="text-xl font-semibold">Esta liga no está disponible</h2>
              <p className="mt-2 max-w-md text-sm leading-6 text-white/55">{error || 'Solicita a XPH una nueva liga para dejar tu opinión.'}</p>
            </div>
          ) : submitted ? (
            ...
            <form onSubmit={submit} className="space-y-7">
              <div className="grid gap-5 sm:grid-cols-2">
                <label className="space-y-2 text-sm font-semibold text-white">
                  ...
                  <input
                    ...
                    className="mt-2 w-full rounded-2xl border border-white/10 bg-black/35 px-4 py-3 text-sm text-white outline-none transition placeholder:text-white/30 focus:border-[#d4af37]/60 focus:ring-2 focus:ring-[#d4af37]/15"
                  />
                </label>
                <label className="space-y-2 text-sm font-semibold text-white">
                  Tipo de evento <span className="font-normal text-white/40">(opcional)</span>
                  <input
                    value={form.eventType}
                    onChange={(event) => setForm((current) => ({ ...current, eventType: event.target.value }))}
                    maxLength={60}
                    placeholder="Boda, XV años, sesión…"
                    className="mt-2 w-full rounded-2xl border border-white/10 bg-black/35 px-4 py-3 text-sm text-white outline-none transition placeholder:text-white/30 focus:border-[#d4af37]/60 focus:ring-2 focus:ring-[#d4af37]/15"
                  />
                </label>
              </div>
              ...
              <label className="block space-y-2 text-sm font-semibold text-white">
                ...
                <textarea
                  ...
                  maxLength={1500}
                  rows={6}
                  placeholder="Puedes contarnos qué te gustó del servicio, la atención, la cobertura y el resultado de tus fotografías."
              ...
              <button
                ...
                className="flex w-full items-center justify-center gap-2 rounded-2xl bg-[#d4af37] px-5 py-3.5 text-sm font-bold text-black transition hover:bg-[#e0bf4c] disabled:cursor-not-allowed disabled:opacity-40"
              >
                {submitting ? <Loader2 className="h-5 w-5 animate-spin" /> : <Send className="h-5 w-5" />}
                {submitting ? 'Guardando opinión…' : 'Enviar mi opinión'}
              </button>
            </form>
          )}
        </section>
        ...
        <p className="mt-5 text-center text-xs leading-5 text-white/35">
          Esta página no aparece en el menú público y solo puede abrirse con una invitación válida de XPH.
        </p>
      </div>
    </main>
  );
