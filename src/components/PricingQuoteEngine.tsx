      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-14">
        <header className="text-center max-w-3xl mx-auto space-y-4">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-[#161C28] border border-[#D4AF37]/30 text-xs font-semibold text-[#D4AF37]">
            <Sparkles className="w-3.5 h-3.5" /><span>COTIZADOR XPH</span>
          </div>
          ...
          <p className="text-gray-300 text-sm sm:text-base">Los importes mostrados aquí corresponden al catálogo público verificado. La disponibilidad se confirma personalmente.</p>
        </header>
        ...
        <div className="flex justify-center">
          <div className="p-1.5 rounded-2xl bg-[#161C28] border border-white/10 inline-flex flex-wrap justify-center gap-2">
            {(Object.keys(categoryLabels) as EventType[]).map((category) => (
              <button key={category} type="button" onClick={() => handleSelectEventType(category)} className={`px-4 py-2.5 rounded-xl text-xs sm:text-sm font-semibold transition-all ${bookingState.eventType === category ? 'gold-gradient-bg text-black font-bold' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}>
        ...
        <div className={`grid gap-8 items-stretch ${currentPackages.length === 1 ? 'max-w-2xl mx-auto' : 'md:grid-cols-2'}`}>
          {currentPackages.map((pkg) => {
            ...
            return (
              <button key={pkg.id} type="button" onClick={() => handleSelectPackage(pkg.id)} className={`relative text-left rounded-2xl p-6 sm:p-8 transition-all ${selected ? 'bg-[#161C28] border-2 border-[#D4AF37] shadow-xl' : 'bg-[#161C28]/80 border border-white/10 hover:border-white/30'}`}>
                ...
                <div className="pt-4 mt-4 border-t border-white/10">
                  {pkg.price > 0 ? <><span className="text-3xl font-extrabold text-white font-mono">${pkg.price.toLocaleString('es-MX')}</span><span className="text-xs text-gray-400 ml-1">MXN</span></> : <span className="text-2xl font-extrabold text-[#D4AF37]">Cotización personalizada</span>}
                </div>
                <ul className="space-y-2 text-xs text-gray-300 mt-5">
        ...
        {!isCustomQuote ? (
          <div className="grid lg:grid-cols-2 gap-8">
            <div className="rounded-2xl bg-[#161C28] border border-white/10 p-6 space-y-5">
              <div><h3 className="text-xl font-bold text-white">Personaliza tu cobertura</h3><p className="text-xs text-gray-400 mt-1">Solo adicionales con precio confirmado.</p></div>
            ...
            <div className="rounded-2xl bg-[#161C28] border border-[#D4AF37]/30 p-6 sm:p-8 flex flex-col justify-between gap-8">
              <div><span className="text-[10px] uppercase tracking-widest text-gray-400 font-mono">Total estimado</span><div className="mt-2"><span className="text-4xl font-black text-[#D4AF37] font-mono">${quotedTotal.toLocaleString('es-MX')}</span><span className="text-sm text-gray-400 ml-2">MXN</span></div><p className="text-xs text-gray-400 mt-3">No confirma ni bloquea fecha.</p></div>
              <button type="button" onClick={handleProceed} className="w-full py-4 rounded-xl gold-gradient-bg text-black font-extrabold text-sm">Solicitar disponibilidad</button>
            </div>
          </div>
        ) : (
