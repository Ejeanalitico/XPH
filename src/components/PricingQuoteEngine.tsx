...
export const PricingQuoteEngine: React.FC<PricingQuoteEngineProps> = ({
  ...
  const packages = PACKAGES_BY_EVENT;
  ...
  const handleSelectEventType = (eventType: EventType) => {
    ...
    onUpdateBookingState((prev) => ({
      ...
      selectedAddons: customQuote ? [] : prev.selectedAddons.filter((id) => addons.some((addon) => addon.id === id)),
  ...
  const handleSelectPackage = (packageId: string) => {
    ...
    onUpdateBookingState((prev) => ({
      ...
      selectedAddons: pkg.price === 0 ? [] : prev.selectedAddons.filter((id) => addons.some((addon) => addon.id === id)),
            ...
            {(Object.keys(categoryLabels) as EventType[]).map((category) => {
              ...
              return (
                <button
                  ...
                  className={`px-4 py-2.5 rounded-xl text-xs sm:text-sm font-semibold transition-all cursor-pointer ${active ? 'gold-gradient-bg text-black font-bold' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}
                >
          ...
          {currentPackages.map((pkg) => {
            ...
            return (
              <button
                ...
                className={`relative text-left rounded-2xl p-6 sm:p-8 transition-all flex flex-col ${selected ? 'bg-[#161C28] border-2 border-[#D4AF37] shadow-xl' : 'bg-[#161C28]/80 border border-white/10 hover:border-white/30'}`}
              >
                ...
                <div className="space-y-5 flex-1">
                  <div>
                    <div className="flex items-start justify-between gap-3">
                      ...
                      {selected && <span className="w-5 h-5 rounded-full bg-[#D4AF37] text-black flex items-center justify-center shrink-0"><Check className="w-3.5 h-3.5 stroke-[3]" /></span>}
                    </div>
                  ...
                  <div className="pt-4 border-t border-white/10">
                    {pkg.price > 0 ? (
                      <><span className="text-3xl font-extrabold text-white font-mono">${pkg.price.toLocaleString('es-MX')}</span><span className="text-xs text-gray-400 ml-1">MXN</span><p className="text-[11px] text-gray-500 mt-1">Precio base publicado.</p></>
                    ) : (
                      <><span className="text-2xl font-extrabold text-[#D4AF37]">Cotización personalizada</span><p className="text-[11px] text-gray-500 mt-1">Se calcula según alcance y necesidades.</p></>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    <ul className="space-y-2 text-xs text-gray-300 mt-2">
                      {pkg.features.map((feature) => <li key={feature} className="flex items-start gap-2"><Check className="w-4 h-4 text-[#D4AF37] shrink-0 mt-0.5" /><span>{feature}</span></li>)}
                    </ul>
                  </div>
                  ...
                  {pkg.notIncludes && pkg.notIncludes.length > 0 && (
                    <div className="pt-3 border-t border-white/10">
                      ...
                      <ul className="space-y-2 text-xs text-gray-400 mt-2">
                        {pkg.notIncludes.map((feature) => <li key={feature} className="flex items-start gap-2"><X className="w-3.5 h-3.5 text-rose-400 shrink-0 mt-0.5" /><span>{feature}</span></li>)}
                      </ul>
                    </div>
                  )}
                </div>
        ...
        {!isCustomQuote ? (
          <div className="grid lg:grid-cols-2 gap-8">
            <div className="rounded-2xl bg-[#161C28] border border-white/10 p-6 space-y-5">
              <div><h3 className="text-xl font-bold text-white">Personaliza tu cobertura</h3><p className="text-xs text-gray-400 mt-1">Adicionales con precio confirmado.</p></div>
              <div className="space-y-3">
                {addons.filter((addon) => addon.type === 'checkbox').map((addon) => {
                  const active = bookingState.selectedAddons.includes(addon.id);
                  return (
                    <button key={addon.id} type="button" onClick={() => handleToggleAddon(addon.id)} className={`w-full p-4 rounded-xl border text-left flex items-start justify-between gap-4 transition-all ${active ? 'border-[#D4AF37] bg-[#D4AF37]/5' : 'border-white/10 bg-[#0B0F17] hover:border-white/20'}`}>
                      <div><p className="text-sm font-semibold text-white">{addon.name}</p><p className="text-xs text-gray-400 mt-1">{addon.description}</p></div>
              ...
              {extraHoursAddon && (
                <div className="p-4 rounded-xl bg-[#0B0F17] border border-white/10 flex items-center justify-between gap-4">
                  <div><p className="text-sm font-semibold text-white">Horas extra</p><p className="text-xs text-gray-400">${extraHoursRate.toLocaleString('es-MX')} MXN por hora completa</p></div>
                  <div className="flex items-center gap-3">
                    <button type="button" aria-label="Quitar hora extra" onClick={() => handleExtraHoursChange(-1)} className="p-2 rounded-lg bg-white/10 text-white"><Minus className="w-4 h-4" /></button>
                    <span className="w-6 text-center text-white font-mono">{bookingState.extraHours}</span>
                    <button type="button" aria-label="Agregar hora extra" onClick={() => handleExtraHoursChange(1)} className="p-2 rounded-lg bg-white/10 text-white"><Plus className="w-4 h-4" /></button>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="rounded-2xl bg-[#161C28] border border-[#D4AF37]/30 p-6 sm:p-8 flex flex-col justify-between gap-8">
              <div>
                ...
                <div className="mt-2"><span className="text-4xl font-black text-[#D4AF37] font-mono">${bookingState.total.toLocaleString('es-MX')}</span><span className="text-sm text-gray-400 ml-2">MXN</span></div>
                <p className="text-xs text-gray-400 mt-3 leading-relaxed">Este cálculo es informativo. La disponibilidad de fecha se confirma directamente antes de cualquier apartado.</p>
              </div>
              <button type="button" onClick={onProceedToBooking} className="w-full py-4 rounded-xl gold-gradient-bg text-black font-extrabold text-sm cursor-pointer">Solicitar disponibilidad</button>
            </div>
          </div>
        ) : (
          <div className="max-w-2xl mx-auto rounded-2xl bg-[#161C28] border border-[#D4AF37]/30 p-6 sm:p-8 text-center space-y-4">
            ...
            <button type="button" onClick={onProceedToBooking} className="w-full sm:w-auto px-8 py-4 rounded-xl gold-gradient-bg text-black font-extrabold text-sm cursor-pointer">Solicitar cotización y disponibilidad</button>
          </div>
        )}
      </div>
    </section>
