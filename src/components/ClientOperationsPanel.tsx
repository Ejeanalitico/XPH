...
interface Props {
  ...
  preContractMode?: boolean;
}

export const ClientOperationsPanel: React.FC<Props> = ({ client, snapshot, onSnapshotChange, onClientPatch, notify, focusedNotificationId = '', canAssignTeam = false, preContractMode = false }) => {
    ...
    try {
      ...
      onSnapshotChange((previous) => ({
        ...
        packageSnapshots: [result.packageSnapshot, ...previous.packageSnapshots
          ...
          .map((item) => client.recordType === 'Prospecto'
            ? item
            : item.clientId === client.id && item.status === 'ACTIVO'
              ? { ...item, status: 'REEMPLAZADO' as const }
              : item)],
        services: [...result.services, ...previous.services.filter((item) => !result.services.some((saved) => saved.id === item.id))],
      }));
      setPackageKey('');
      notify(client.recordType === 'Prospecto'
        ? 'Opción de paquete guardada. Puedes agregar más paquetes al mismo prospecto.'
  ...
  const removeAddon = async (addon: ClientAddon) => {
    if (busy) return;
  ...
  const patchClientWithLoading = async (patch: Partial<CrmClient>) => {
    if (busy) return;
    setBusy(true);
    try {
      await onClientPatch(patch);
    } catch (error: any) {
      notify(error?.message || 'No se pudo actualizar el expediente.');
    } finally {
      setBusy(false);
    }
  };
  ...
  return <div className="space-y-5 border-t border-white/10 p-5 sm:p-8">
    {(busy || Boolean(notificationBusyId)) && <div className="fixed inset-0 z-[260] grid place-items-center bg-black/70 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-live="assertive" aria-label="Procesando"><div className="w-full max-w-sm rounded-2xl border border-[#D4AF37]/30 bg-[#111722] p-6 text-center shadow-2xl"><Loader2 className="mx-auto h-9 w-9 animate-spin text-[#D4AF37]" /><h3 className="mt-4 text-base font-bold text-white">Procesando cambios</h3><p className="mt-2 text-sm leading-6 text-gray-400">Espera un momento. La información se está guardando o actualizando.</p></div></div>}
    {preContractMode && <section className="rounded-2xl border border-[#D4AF37]/30 bg-[#D4AF37]/[.07] p-5"><p className="text-xs font-semibold uppercase tracking-[.16em] text-[#F5D76E]">Preparación comercial del prospecto</p><h3 className="mt-2 font-semibold text-white">Paquete y servicios solicitados</h3><p className="mt-1 text-sm leading-6 text-gray-300">Completa lo que están solicitando antes de generar la cotización o el contrato. Los adicionales son opcionales: si no aplican, deja esa sección vacía y continúa. Esto no convierte todavía al prospecto en cliente.</p></section>}
