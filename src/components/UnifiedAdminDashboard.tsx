                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-3">
                  {selectedGalleryMedia.map((item) => (
                    <div key={`${item.id}-${item.mediaType}`} className="relative rounded-xl overflow-hidden border border-white/10 bg-[#0B0F17]">
                      {item.mediaType === 'video' ? <div className="aspect-square flex items-center justify-center"><FileVideo2 className="w-10 h-10 text-[#D4AF37]" /></div> : <SafeImage src={item.url} alt={item.title} className="w-full aspect-square object-cover" />}
