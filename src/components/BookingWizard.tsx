  ...
  const handleSendRequest = async () => {
    ...
    try {
      ...
      if (bookingState.extraHours > 0) {
        activeAddonNames.push(`${bookingState.extraHours} Horas Extra de Cobertura`);
      }
      ...
      const nextQuotes = [requestRecord, ...existingQuotes];
      ...
      if (!saved) {
        throw new Error('No se pudo registrar la solicitud en Google Sheets.');
      }

      const packagePrice = selectedPackage.price > 0
        ? `$${selectedPackage.price.toLocaleString('es-MX')} MXN`
        : 'Cotización personalizada';
      const totalText = bookingState.total > 0
      ...
      onShowToast(
        ...
        'Se guardó en Google Sheets y se preparó el mensaje de WhatsApp. La fecha sigue pendiente de confirmación.',
        'success'
      );
    } catch (error: any) {
      ...
      onShowToast(
        ...
        error?.message || 'Revisa la conexión con Apps Script e intenta nuevamente.',
        'warning'
      );
    } finally {
      setSending(false);
            ...
            <div className="space-y-6">
              ...
              <div className="grid sm:grid-cols-2 gap-6 items-stretch">
                <div className="p-5 rounded-2xl bg-[#0B0F17] border border-white/10 space-y-2">
                  <label className="text-xs font-semibold text-gray-300 block">Fecha tentativa del evento *</label>
                  <input
                    type="date"
                  ...
                  {bookingState.date ? (
                    <>
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-400">{bookingState.date}. Pendiente de validación en agenda.</p>
                      </div>
                    </>
                  ) : (
                    <>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-4 border-t border-white/10">
                <button type="button" onClick={handleCopyRequestLink} className="px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-gray-300 hover:text-white text-xs font-semibold flex items-center justify-center gap-2 cursor-pointer">
                  <Share2 className="w-4 h-4 text-[#D4AF37]" />
                  <span>{copiedLink ? 'Enlace copiado' : 'Copiar enlace de solicitud'}</span>
                </button>
                <button type="button" onClick={() => setCurrentStep(2)} disabled={!bookingState.date} className="px-6 py-3 rounded-xl gold-gradient-bg text-black font-bold text-xs flex items-center justify-center gap-2 disabled:opacity-40 cursor-pointer">
                  <span>Continuar con mis datos</span><ArrowRight className="w-4 h-4" />
                </button>
              </div>
            </div>
          )}
          ...
          {currentStep === 2 && (
            <div className="space-y-6">
              <div className="space-y-1">
                ...
                <p className="text-xs text-gray-400">Estos datos se usan para revisar tu solicitud y continuar la atención por WhatsApp.</p>
              </div>
              ...
              <div className="p-5 rounded-2xl bg-[#0B0F17] border border-white/10 space-y-3 text-xs">
                ...
                <div className="flex justify-between gap-4"><span className="text-gray-400">Precio</span><span className="text-[#D4AF37] font-mono font-bold text-right">{selectedPackage.price > 0 ? `$${selectedPackage.price.toLocaleString('es-MX')} MXN` : 'Cotización personalizada'}</span></div>
                <div className="flex justify-between gap-4"><span className="text-gray-400">Fecha tentativa</span><span className="text-white font-mono">{bookingState.date || 'Por definir'}</span></div>
              </div>

              <div className="grid sm:grid-cols-2 gap-4">
                <div>
                  <label className="text-xs font-semibold text-gray-300 block mb-1">Nombre completo *</label>
                  <input type="text" value={bookingState.clientName} onChange={(event) => onUpdateBookingState((prev) => ({ ...prev, clientName: event.target.value }))} placeholder="Nombre completo" className="w-full px-4 py-2.5 rounded-xl bg-[#0B0F17] border border-white/15 text-white focus:outline-none focus:border-[#D4AF37] text-xs" />
                </div>
                <div>
                  <label className="text-xs font-semibold text-gray-300 block mb-1">Correo electrónico *</label>
                  <input type="email" value={bookingState.clientEmail} onChange={(event) => onUpdateBookingState((prev) => ({ ...prev, clientEmail: event.target.value }))} placeholder="correo@ejemplo.com" className="w-full px-4 py-2.5 rounded-xl bg-[#0B0F17] border border-white/15 text-white focus:outline-none focus:border-[#D4AF37] text-xs" />
                </div>
                <div>
                  <label className="text-xs font-semibold text-gray-300 block mb-1">WhatsApp *</label>
                  <input type="tel" value={bookingState.clientPhone} onChange={(event) => onUpdateBookingState((prev) => ({ ...prev, clientPhone: event.target.value }))} placeholder="+52 55 0000 0000" className="w-full px-4 py-2.5 rounded-xl bg-[#0B0F17] border border-white/15 text-white focus:outline-none focus:border-[#D4AF37] text-xs" />
                </div>
                <div>
                  <label className="text-xs font-semibold text-gray-300 block mb-1">Ciudad / locación</label>
                  <input type="text" value={bookingState.eventCity} onChange={(event) => onUpdateBookingState((prev) => ({ ...prev, eventCity: event.target.value }))} placeholder="Ej. Iztacalco, CDMX" className="w-full px-4 py-2.5 rounded-xl bg-[#0B0F17] border border-white/15 text-white focus:outline-none focus:border-[#D4AF37] text-xs" />
                </div>
              </div>

              <div>
                <label className="text-xs font-semibold text-gray-300 block mb-1">Notas adicionales</label>
                <textarea value={bookingState.notes} onChange={(event) => onUpdateBookingState((prev) => ({ ...prev, notes: event.target.value }))} rows={3} placeholder="Cuéntanos brevemente qué necesitas para tu evento." className="w-full px-4 py-3 rounded-xl bg-[#0B0F17] border border-white/15 text-white focus:outline-none focus:border-[#D4AF37] text-xs resize-y" />
              </div>
              ...
              <div className="flex flex-col-reverse sm:flex-row sm:items-center justify-between gap-3 pt-4 border-t border-white/10">
                <button type="button" onClick={() => setCurrentStep(1)} className="px-5 py-2.5 rounded-xl bg-white/5 text-gray-300 text-xs font-semibold flex items-center justify-center gap-2 cursor-pointer"><ArrowLeft className="w-4 h-4" /><span>Regresar</span></button>
                <button type="button" onClick={handleSendRequest} disabled={!bookingState.clientName || !bookingState.clientEmail || !bookingState.clientPhone || sending} className="px-6 py-3 rounded-xl gold-gradient-bg text-black font-bold text-xs flex items-center justify-center gap-2 disabled:opacity-40 cursor-pointer">
                  {sending ? <Loader2 className="w-4 h-4 animate-spin" /> : <MessageCircle className="w-4 h-4" />}
                  <span>{sending ? 'Registrando solicitud…' : 'Enviar solicitud por WhatsApp'}</span>
                </button>
              </div>
            </div>
          )}
