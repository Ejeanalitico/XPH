...
import { loadSiteDataFromCloud, saveSiteDataToCloud } from '../utils/googleDrive';
...
export const BookingWizard: React.FC<BookingWizardProps> = ({ bookingState, onUpdateBookingState, onShowToast }) => {
  const [currentStep, setCurrentStep] = useState<1 | 2>(1);
  const [copiedLink, setCopiedLink] = useState(false);
  const [sending, setSending] = useState(false);

  const currentPackages = PACKAGES_BY_EVENT[bookingState.eventType] || PACKAGES_BY_EVENT.bodas;
  const currentAddons = ADDONS_CATALOG;
  ...
  const handleDateChange = (newDate: string) => {
    ...
    if (newDate) onShowToast('Fecha seleccionada', 'La disponibilidad debe confirmarse en la agenda antes de apartar el evento.', 'info');
  };
  ...
  const handleCopyRequestLink = async () => {
    ...
    setCopiedLink(true);
    onShowToast(success ? 'Enlace copiado' : 'Enlace de solicitud', success ? 'Puedes compartir este enlace para solicitar disponibilidad.' : link, 'info');
  ...
  const handleSendRequest = async () => {
    ...
    if (!bookingState.clientName || !bookingState.clientEmail || !bookingState.clientPhone) {
      setCurrentStep(2);
      onShowToast('Campos incompletos', 'Completa nombre, correo y WhatsApp antes de enviar la solicitud.', 'warning');
      return;
    }
    if (sending) return;
    setSending(true);
    ...
    try {
      ...
      const existingQuotes = Array.isArray(cloudConfig.quotes) ? cloudConfig.quotes : [];
      ...
      const saved = await saveSiteDataToCloud(
        { quotes: [requestRecord, ...existingQuotes] },
        'NUEVA_SOLICITUD_DISPONIBILIDAD',
        `Solicitud web de ${bookingState.clientName} para ${bookingState.date}`
      );
      if (!saved) throw new Error('No se pudo registrar la solicitud en Google Sheets.');
      ...
      onShowToast('Solicitud registrada', 'Se guardó en Google Sheets y se preparó el mensaje de WhatsApp. La fecha sigue pendiente de confirmación.', 'success');
    } catch (error: any) {
      whatsappWindow?.close();
      onShowToast('No se pudo enviar la solicitud', error?.message || 'Revisa la conexión con Apps Script e intenta nuevamente.', 'warning');
    } finally {
      setSending(false);
    }
  };
    ...
    <section id="solicitud" className="py-20 bg-[#0B0F17] relative border-b border-white/5">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10">
        <div className="text-center space-y-3">
          ...
          <p className="text-gray-300 text-sm max-w-2xl mx-auto">Selecciona tu fecha tentativa y comparte tus datos. La disponibilidad se confirma personalmente antes de cualquier apartado.</p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2 sm:gap-4 p-2 rounded-2xl bg-[#161C28] border border-white/10">
          {[
            ...
            return (
              <button key={item.step} type="button" onClick={() => setCurrentStep(item.step)} className={`p-3 rounded-xl flex items-center justify-center gap-2 text-xs font-bold transition-all ${active ? 'gold-gradient-bg text-black' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}>
        ...
        <div className="p-5 sm:p-8 lg:p-10 rounded-2xl bg-[#161C28] border border-white/10 shadow-2xl">
          {currentStep === 1 ? (
            <div className="space-y-6">
              <div>
                <h3 className="text-xl font-bold font-serif-luxury text-white flex items-center gap-2"><CalendarIcon className="w-5 h-5 text-[#D4AF37]" />Fecha tentativa del evento</h3>
                <p className="text-xs text-gray-400 mt-1">Elegir una fecha aquí no la bloquea ni confirma.</p>
              </div>
              <div className="grid sm:grid-cols-2 gap-6">
                <div className="p-5 rounded-2xl bg-[#0B0F17] border border-white/10">
                  <label className="text-xs font-semibold text-gray-300 block mb-2">Fecha tentativa *</label>
                  <input type="date" value={bookingState.date} onChange={(e) => handleDateChange(e.target.value)} className="w-full px-4 py-3 rounded-xl bg-[#161C28] border border-white/20 text-white focus:outline-none focus:border-[#D4AF37] font-mono text-sm" />
                </div>
                <div className="p-5 rounded-2xl bg-[#0B0F17] border border-white/10 flex items-center gap-3">
                  {bookingState.date ? <><CheckCircle2 className="w-5 h-5 text-[#D4AF37]" /><div><p className="text-sm font-bold text-white">Fecha seleccionada</p><p className="text-xs text-gray-400">{bookingState.date} · pendiente de validación.</p></div></> : <><AlertCircle className="w-5 h-5 text-gray-500" /><p className="text-xs text-gray-400">Selecciona una fecha para continuar.</p></>}
                </div>
              </div>
              <div className="flex flex-col sm:flex-row justify-between gap-3 pt-4 border-t border-white/10">
                <button type="button" onClick={handleCopyRequestLink} className="px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-gray-300 text-xs font-semibold flex items-center justify-center gap-2"><Share2 className="w-4 h-4 text-[#D4AF37]" />{copiedLink ? 'Enlace copiado' : 'Copiar enlace de solicitud'}</button>
                <button type="button" onClick={() => setCurrentStep(2)} disabled={!bookingState.date} className="px-6 py-3 rounded-xl gold-gradient-bg text-black font-bold text-xs flex items-center justify-center gap-2 disabled:opacity-40">Continuar con mis datos<ArrowRight className="w-4 h-4" /></button>
              </div>
            </div>
          ) : (
            <div className="space-y-6">
              <div>
                <h3 className="text-xl font-bold font-serif-luxury text-white flex items-center gap-2"><FileText className="w-5 h-5 text-[#D4AF37]" />Datos de contacto</h3>
                <p className="text-xs text-gray-400 mt-1">Se guardan en Google Sheets mediante Apps Script para dar seguimiento a tu solicitud.</p>
              </div>

              <div className="p-5 rounded-2xl bg-[#0B0F17] border border-white/10 space-y-2 text-xs">
                <div className="flex justify-between gap-4"><span className="text-gray-400">Paquete</span><span className="text-white font-semibold text-right">{selectedPackage.name}</span></div>
                <div className="flex justify-between gap-4"><span className="text-gray-400">Precio</span><span className="text-[#D4AF37] font-mono font-bold">{selectedPackage.price > 0 ? `$${selectedPackage.price.toLocaleString('es-MX')} MXN` : 'Cotización personalizada'}</span></div>
                <div className="flex justify-between gap-4"><span className="text-gray-400">Fecha</span><span className="text-white font-mono">{bookingState.date}</span></div>
              </div>
              ...
              <div className="flex flex-col-reverse sm:flex-row justify-between gap-3 pt-4 border-t border-white/10">
                <button type="button" onClick={() => setCurrentStep(1)} className="px-5 py-2.5 rounded-xl bg-white/5 text-gray-300 text-xs font-semibold flex items-center justify-center gap-2"><ArrowLeft className="w-4 h-4" />Regresar</button>
                <button type="button" onClick={handleSendRequest} disabled={!bookingState.clientName || !bookingState.clientEmail || !bookingState.clientPhone || sending} className="px-6 py-3 rounded-xl gold-gradient-bg text-black font-bold text-xs flex items-center justify-center gap-2 disabled:opacity-40">{sending ? <Loader2 className="w-4 h-4 animate-spin" /> : <MessageCircle className="w-4 h-4" />}{sending ? 'Registrando solicitud…' : 'Enviar solicitud por WhatsApp'}</button>
              </div>
            </div>
          )}
        </div>
