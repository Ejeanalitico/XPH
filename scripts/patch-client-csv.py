...
anchor = "const money = (value: number) => new Intl.NumberFormat('es-MX', { style: 'currency', currency: 'MXN' }).format(Number(value) || 0);\n"
helpers = r"""const clientCsvColumns: Array<[keyof CrmClient, string]> = [
  ['id', 'ID'],
...
method_anchor = "  const saveExpense = async (event: React.FormEvent) => {\n"
import_method = r"""  const importClientsCsv = async (file: File) => {
...
if import_method not in text:
    if method_anchor not in text:
        raise SystemExit('saveExpense anchor not found')
    text = text.replace(method_anchor, import_method + method_anchor, 1)

old_header = """          <div className=\"flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between\">
...
new_header = """          <div className=\"flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between\">
            <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={`Buscar ${tab === 'prospects' ? 'prospectos' : 'clientes'} por nombre, teléfono, evento o estado`} className=\"w-full max-w-xl rounded-xl border border-white/10 bg-[#161C28] px-4 py-3 text-sm\" />
...
if new_header not in text:
    if old_header not in text:
        raise SystemExit('client list header anchor not found')
    text = text.replace(old_header, new_header, 1)

old_detail_actions = '<div className="flex flex-wrap gap-2">{canEditSelected && selectedClient.recordType === \'Prospecto\''
new_detail_actions = '<div className="flex flex-wrap gap-2">{session.role === \'SUPER_ADMIN\' && selectedClient.recordType === \'Cliente\' && <button type="button" onClick={() => downloadClientCsv([selectedClient], snapshot)} className="inline-flex items-center gap-2 rounded-lg border border-emerald-300/30 bg-emerald-400/10 px-4 py-2 text-sm font-semibold text-emerald-100"><Download className="h-4 w-4" />Descargar CSV</button>}{canEditSelected && selectedClient.recordType === \'Prospecto\''
if new_detail_actions not in text:
    if old_detail_actions not in text:
        raise SystemExit('selected client actions anchor not found')
    text = text.replace(old_detail_actions, new_detail_actions, 1)
